...
#define LCD_OPCODE_READ_CMD (0x0BULL) // espressif
// #define LCD_OPCODE_READ_CMD (0x03ULL) // datasheet
#define LCD_OPCODE_WRITE_COLOR (0x32ULL)

static const char *TAG = "atomic_lcd_axs";
...
static esp_err_t panel_axs_set_gap(esp_lcd_panel_t *panel, int x_gap, int y_gap);
...
esp_err_t atomic_lcd_new_panel_axs(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel) {
    ...
    axs->io = io;
    axs->fb_bits_per_pixel = fb_bits_per_pixel;
    ...
    axs->base.init = panel_axs_init;
    axs->base.draw_bitmap = panel_axs_draw_bitmap;
    axs->base.invert_color = panel_axs_invert_color;
    axs->base.set_gap = panel_axs_set_gap;
    axs->base.mirror = panel_axs_mirror;
    axs->base.swap_xy = panel_axs_swap_xy;
    axs->base.disp_on_off = panel_axs_disp_off;
    *ret_panel = &(axs->base);
    ESP_LOGD(TAG, "new axs panel @%p", axs);
...
static esp_err_t panel_axs_del(esp_lcd_panel_t *panel) {
    ...
    if (axs->reset_gpio_num >= 0) {
        gpio_reset_pin(axs->reset_gpio_num);
    }
    ESP_LOGD(TAG, "del axs panel @%p", axs);
    free(axs);
    return ESP_OK;
}
...
static esp_err_t panel_axs_init(esp_lcd_panel_t *panel) {
    ...
    ESP_RETURN_ON_ERROR(tx_param(axs, io, LCD_CMD_SLPOUT, NULL, 0), TAG, "send command failed");
    vTaskDelay(pdMS_TO_TICKS(100));
    ESP_RETURN_ON_ERROR(tx_param(axs, io, LCD_CMD_MADCTL,
                                 (uint8_t[]){
                                     axs->madctl_val,
                                 },
                                 1),
                        TAG, "send command failed");
    ESP_RETURN_ON_ERROR(tx_param(axs, io, LCD_CMD_COLMOD,
                                 (uint8_t[]){
                                     axs->colmod_val,
                                 },
                                 1),
    ...
    const axs_lcd_init_cmd_t *init_cmds = NULL;
    uint16_t init_cmds_size = 0;
    if (axs->init_cmds) {
        init_cmds = axs->init_cmds;
        init_cmds_size = axs->init_cmds_size;
    } else {
        init_cmds = vendor_specific_init_default;
        init_cmds_size = sizeof(vendor_specific_init_default) / sizeof(axs_lcd_init_cmd_t);
    }
    ...
    for (int i = 0; i < init_cmds_size; i++) {
        ...
        case LCD_CMD_MADCTL:
            ...
            axs->madctl_val = ((uint8_t *)init_cmds[i].data)[0];
            break;
        case LCD_CMD_COLMOD:
            is_cmd_overwritten = true;
            axs->colmod_val = ((uint8_t *)init_cmds[i].data)[0];
            break;
        default:
...
static esp_err_t panel_axs_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end, const void *color_data) {
    ...
    if (0 == axs->flags.use_qspi_interface) {
        tx_param(axs, io, LCD_CMD_RASET,
                 (uint8_t[]){
                     (y_start >> 8) & 0xFF,
                     y_start & 0xFF,
                     ((y_end - 1) >> 8) & 0xFF,
    ...
    size_t len = (x_end - x_start) * (y_end - y_start) * axs->fb_bits_per_pixel / 8;
    if (y_start == 0) {
        tx_color(axs, io, LCD_CMD_RAMWR, color_data, len); // 2C
    } else {
        tx_color(axs, io, LCD_CMD_RAMWRC, color_data, len); // 3C
    }
...
static esp_err_t panel_axs_invert_color(esp_lcd_panel_t *panel, bool invert_color_data) {
    ...
    tx_param(axs, io, command, NULL, 0);
    return ESP_OK;
}
...
static esp_err_t panel_axs_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y) {
    ...
    esp_lcd_panel_io_handle_t io = axs->io;
    if (mirror_x) {
        axs->madctl_val |= LCD_CMD_MX_BIT;
    } else {
        axs->madctl_val &= ~LCD_CMD_MX_BIT;
    }
    if (mirror_y) {
        axs->madctl_val |= LCD_CMD_MY_BIT;
    } else {
        axs->madctl_val &= ~LCD_CMD_MY_BIT;
    }
    tx_param(axs, io, LCD_CMD_MADCTL, (uint8_t[]){axs->madctl_val}, 1);
    return ESP_OK;
}
...
esp_err_t atomic_lcd_touch_new_i2c_axs(const esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *tp) {
    ...
    esp_err_t ret = ESP_OK;
    esp_lcd_touch_handle_t axs = calloc(1, sizeof(esp_lcd_touch_t));
    ESP_GOTO_ON_FALSE(axs, ESP_ERR_NO_MEM, err, TAG, "Touch handle malloc failed");

    /* Communication interface */
    axs->io = io;
    /* Only supported callbacks are set */
    axs->read_data = touch_axs_read_data;
    axs->get_xy = touch_axs_get_xy;
    axs->del = touch_axs_del;
    /* Mutex */
    axs->data.lock.owner = portMUX_FREE_VAL;
    /* Save config */
    memcpy(&axs->config, config, sizeof(esp_lcd_touch_config_t));
    ...
    if (axs->config.int_gpio_num != GPIO_NUM_NC) {
        ...
        if (axs->config.interrupt_callback) {
            esp_lcd_touch_register_interrupt_callback(axs, axs->config.interrupt_callback);
        }
    }
    ...
    /* Reset controller */
    ESP_GOTO_ON_ERROR(touch_axs_reset(axs), err, TAG, "Reset failed");
    *tp = axs;
...
// New Commands for QSPI interface ///////////////////////////////////

uint32_t axs_tx_cmd_fmt(uint8_t cmd) {
    uint32_t qspi_cmd = (uint32_t)(cmd & 0xFF);
    qspi_cmd <<= 8;
    qspi_cmd |= (uint32_t)LCD_OPCODE_WRITE_CMD << 24;
    return qspi_cmd;
}
...
uint32_t axs_rx_cmd_fmt(uint8_t cmd) {
    uint32_t qspi_cmd = (uint32_t)(cmd & 0xFF);
    qspi_cmd <<= 8;
    qspi_cmd |= (uint32_t)LCD_OPCODE_READ_CMD << 24;
    return qspi_cmd;
}
...
esp_err_t axs_rx_cmd(esp_lcd_panel_io_handle_t io, uint8_t cmd, void *param, size_t param_size) {
    uint32_t qspi_rx_cmd = axs_rx_cmd_fmt(cmd);
    return io->rx_param(io, qspi_rx_cmd, param, param_size);
}

esp_err_t axs_test_invert(esp_lcd_panel_io_handle_t io, bool invert_color_data) {
    return axs_tx_cmd(io, invert_color_data ? LCD_CMD_INVON : LCD_CMD_INVOFF, NULL, 0);
}
...
void axs_fill_rect_rgb565(esp_lcd_panel_t *panel, int x1, int y1, int x2, int y2, uint16_t color565) {
    ...
    if (!axs->flags.use_qspi_interface) {
        uint8_t raset[4] = {(y1 >> 8) & 0xFF, y1 & 0xFF, ((y2 - 1) >> 8) & 0xFF, (y2 - 1) & 0xFF};
        tx_param(axs, io, LCD_CMD_RASET, raset, 4);
    }

    int w = x2 - x1, h = y2 - y1;
    size_t pixels = (size_t)w * (size_t)h;

    uint16_t tile_px[256];
    for (int i = 0; i < 256; ++i)
        tile_px[i] = color565;

    size_t first = pixels > 256 ? 256 : pixels;
    tx_color(axs, io, LCD_CMD_RAMWR, tile_px, first * sizeof(tile_px[0]));
    pixels -= first;

    while (pixels) {
        size_t chunk = pixels > 256 ? 256 : pixels;
        tx_color(axs, io, LCD_CMD_RAMWRC, tile_px, chunk * sizeof(tile_px[0]));
        pixels -= chunk;
    }
}
...
void axs_fill_rect_rgb888_hw(esp_lcd_panel_t *panel, int x1, int y1, int x2, int y2, uint8_t r, uint8_t g, uint8_t b) {
    ...
    // Rows for partial window (PTLAR expects Y start/end)
    uint8_t ptlar[4] = {(y1 >> 8) & 0xFF, y1 & 0xFF, ((y2 - 1) >> 8) & 0xFF, (y2 - 1) & 0xFF};
    tx_param(axs, io, 0x30, ptlar, 4); // PTLAR
    tx_param(axs, io, 0x12, NULL, 0);  // PTLON

    uint8_t rgb[3] = {r, g, b};
    tx_param(axs, io, 0x2F, rgb, 3); // RAWFILL (fills the partial window)

    tx_param(axs, io, 0x13, NULL, 0); // NORON (exit partial)
}
*/