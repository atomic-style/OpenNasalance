...
esp_err_t atomic_touch_init(const atomic_touch_cfg_t *cfg) {
  ...
  notice(TAG, "init FT6336G  i2c%d sda=%d scl=%d rst=%d int=%d  rot=%d",
         cfg->i2c_port, cfg->pin_sda, cfg->pin_scl, cfg->pin_rst, cfg->pin_int,
         cfg->rotation_ccw);
  ...
  if (r != ESP_OK) {
    warn(TAG, "FT6336G DEV_MODE write failed: %s (continuing)",
         esp_err_to_name(r));
  }
...
esp_err_t atomic_touch_read(atomic_touch_point_t *out) {
  ...
  out->y = y;
  out->pressed = true;
  info(TAG, "atomic_touch_read(%d, %d)", x, y);
  return ESP_OK;
}

i2c_master_bus_handle_t atomic_touch_get_bus(void) { return s_bus; }
