...
static esp_err_t dump_value_for_entry(nvs_handle_t handle, const char *ns,
  ...
  case NVS_TYPE_U64: {
    ...
    info(TAG, "NVS %s: key='%s' type=U64 val=%llu", ns, key,
  ...
  case NVS_TYPE_STR: {
    ...
    if (len < 128) {
      ...
      info(TAG, "NVS %s: key='%s' type=STR len=%u val='%s'", ns, key,
           (unsigned)len, buf);
    } else {
      info(TAG, "NVS %s: key='%s' type=STR len=%u (too long to dump)", ns, key,
           (unsigned)len);
    }
    break;
  }
  case NVS_TYPE_BLOB: {
    ...
    if (len < 128) {
      ...
      info(TAG, "NVS %s: key='%s' type=BLOB len=%u val='%s'", ns, key,
           (unsigned)len, (char *)buf);
    } else {
      info(TAG, "NVS %s: key='%s' type=BLOB len=%u (too long to dump)", ns, key,
           (unsigned)len);
    }
    break;
  }
  case NVS_TYPE_ANY: {
    ...
    if (len < 128) {
      try(nvs_get_blob(handle, key, buf, &len));
      info(TAG, "NVS %s: key='%s' type=ANY len=%u val='%s'", ns, key,
           (unsigned)len, (char *)buf);
    } else {
      info(TAG, "NVS %s: key='%s' type=ANY len=%u (too long to dump)", ns, key,
           (unsigned)len);
    }
    break;
  }
...
static esp_err_t a_nvs_dump_all(void) {
  ...
  while (err == ESP_OK) {
    ...
    if (ns_handle)
      dump_value_for_entry(ns_handle, info_entry.namespace_name, info_entry.key,
                           info_entry.type);
...
static esp_err_t a_nvs_stats(void) {
  ...
  // info(TAG, "Used: %lu, Free: %lu, Available: %lu, All: %lu, Namespaces:
  // %lu", nvs_stats.used_entries,
  //      nvs_stats.free_entries, nvs_stats.available_entries,
...
esp_err_t a_nvs_erase_key(const char *key) {
  return nvs_erase_key(a_handle, key);
}

esp_err_t a_nvs_find_key(const char *key, nvs_type_t *type) {
  return nvs_find_key(a_handle, key, type);
}
