...
static esp_err_t a_ha_config_topics(void) {
    ...
    if (strlen(s_ha_config.mqtt_topic) + strlen(s_ha_config.device_unit) + 12 > topic_max_len) {
        err(TAG, "MQTT topic or device unit too long");
        return ESP_ERR_INVALID_ARG;
    }

    s_topic_availability = heap_caps_malloc(topic_max_len, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    s_topic_state = heap_caps_malloc(topic_max_len, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    s_topic_discovery = heap_caps_malloc(topic_max_len, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    s_topic_cmd = heap_caps_malloc(topic_max_len, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);

    snprintf(s_topic_availability, topic_max_len, "%s/device/%s/availability", s_ha_config.mqtt_topic,
             s_ha_config.device_unit);
    snprintf(s_topic_state, topic_max_len, "%s/device/%s/state", s_ha_config.mqtt_topic, s_ha_config.device_unit);
    snprintf(s_topic_discovery, topic_max_len, "%s/device/%s/config", s_ha_config.mqtt_topic, s_ha_config.device_unit);
    snprintf(s_topic_cmd, topic_max_len, "%s/device/%s/cmd", s_ha_config.mqtt_topic, s_ha_config.device_unit);
...
static esp_err_t ha_buffer_append(const char *str) {
    ...
    if (s_ha_buffer_len + str_len >= HA_BUFFER_SIZE - 1) {
        warn(TAG, "Buffer overflow: s_ha_buffer_len=%d, str_len=%d", s_ha_buffer_len, str_len);
...
static void ha_buffer_append_json(const char *key, const char *value, bool comma) {
    ha_buffer_append("\"");
...
static esp_err_t ha_buffer_clear(void) {
    if (!s_ha_buffer) {
        s_ha_buffer = heap_caps_malloc(HA_BUFFER_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (!s_ha_buffer) {
...
static esp_err_t a_ha_publish_discovery(void) {
    ...
    if (a_sensors_count() > 0) {
        ...
        for (int i = 0; i < a_sensors_count(); i++) {
            ...
            char unique_id[128];
            snprintf(unique_id, sizeof(unique_id), "%s_%s", s_ha_config.device_unit, sensor->unique_id);
            ...
            snprintf(value_template, sizeof(value_template), "{{ value_json.%s }}", sensor->unique_id);
...
esp_err_t a_ha_config(a_ha_config_t *config) {
    if (!config || !config->device_name || !config->device_unit || !config->device_model || !config->device_chip_id) {
        return ESP_ERR_INVALID_ARG;
    }
    s_ha_config = *config;
    return ESP_OK;
